		...
		while( have_posts() ) {
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class();?>>
				<header class="entry-header">
					<h3><?php echo the_title_attribute('echo=0'); ?></h3>
			...
			if($disable_entry_format =='show' || $disable_entry_format =='show-button' || $disable_entry_format =='hide-button'){ ?>
				<footer class="entry-footer">
					...
					<div class="entry-meta">
						...
						<!-- AUTHOR -->
						<span class="author vcard"><p><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" title="<?php the_author(); ?>"><i class="fa fa-heart-o"></i>
						<?php the_author(); ?> </a></span> 
						<!-- DATE -->
						<span class="posted-on"><a title="<?php echo esc_attr( get_the_time() ); ?>" href="<?php the_permalink(); ?>"><i class=""></i>
					...
					<?php esc_html_e('Catégorie(s) : ','edge');  the_category(', '); ?>
					</span> <!-- end .cat-links -->
					<?php $tag_list = get_the_tag_list( 'Tag(s) : ', __( ', ', 'edge' ) );
						if(!empty($tag_list)){ ?>
